...
import org.wso2.carbon.device.mgt.core.operation.timeout.task.impl.OperationTimeoutTask;
...
public class OperationTimeoutTaskManagerServiceImpl implements OperationTimeoutTaskManagerService {
    ...
    @Override
    public void startTask(DeviceType deviceType, OperationTimeout config)
            throws OperationTimeoutTaskException {
        log.info("Operation timeout task adding for " + deviceType.getName());
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Operation timeout task is started for the device type " + deviceType.getName());
                log.debug(
            ...
            String taskName = OPERATION_TIMEOUT_TASK + "_" + deviceType.getName() + "_" + deviceType.getId();
            ...
            if (!taskManager.isTaskScheduled(taskName)) {
                ...
                throw new OperationTimeoutTaskException(
                        "Operation Timeout task is already started for this device-type : " + deviceType.getName());
            }
    ...
    @Override
    public void stopTask(DeviceType deviceType, OperationTimeout config)
            throws OperationTimeoutTaskException {
        try {
            TaskService taskService = DeviceManagementDataHolder.getInstance().getTaskService();
            String taskName = OPERATION_TIMEOUT_TASK + "_" + deviceType.getName() + "_" + deviceType.getId();
    ...
    @Override
    public void updateTask(DeviceType deviceType, OperationTimeout config)
            throws OperationTimeoutTaskException {
        try {
            TaskService taskService = DeviceManagementDataHolder.getInstance().getTaskService();
            TaskManager taskManager = taskService.getTaskManager(OPERATION_TIMEOUT_TASK);
            String taskName = OPERATION_TIMEOUT_TASK + "_" + deviceType + "_" + deviceType.getId();
            if (taskManager.isTaskScheduled(taskName)) {
                taskManager.deleteTask(taskName);
                TaskInfo.TriggerInfo triggerInfo = new TaskInfo.TriggerInfo();
                triggerInfo.setIntervalMillis(config.getTimeout() * 60 * 1000);
                ...
                throw new OperationTimeoutTaskException(
                        "Operation timeout task has not been started for this device-type " +
                                deviceType.getName() + ". Please start the task first.");
            }

        } catch (TaskException e) {
            throw new OperationTimeoutTaskException("Error occurred while updating the Operation timeout  " +
                    "task for device-type : " + deviceType.getName(), e);
        }
    }

    @Override
    public boolean isTaskScheduled(DeviceType deviceType) throws OperationTimeoutTaskException {
        String taskName = OPERATION_TIMEOUT_TASK + "_" + deviceType.getName() + "_" + deviceType.getId();
        TaskService taskService = DeviceManagementDataHolder.getInstance().getTaskService();
