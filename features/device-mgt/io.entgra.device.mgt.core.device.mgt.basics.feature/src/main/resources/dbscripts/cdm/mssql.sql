...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[DM_NOTIFICATION_DEVICE]') AND TYPE IN (N'U'))
...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[DM_NOTIFICATION_USER_ACTION]') AND TYPE IN (N'U'))
...
CREATE TABLE IF NOT EXISTS DM_DEVICE_TYPE_META (
    ...
    META_VALUE NVARCHAR(20000),
    LAST_UPDATED_TIMESTAMP BIGINT NOT NULL,
    TENANT_ID INT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT FK_DEVICE_TYPE FOREIGN KEY (DEVICE_TYPE_ID) REFERENCES DM_DEVICE_TYPE(ID),
...
CREATE TABLE DM_OPERATION_DETAILS (
    ...
    CONSTRAINT UQ_OPERATION_DEVICE UNIQUE (OPERATION_CODE, DEVICE_TYPE)
);
