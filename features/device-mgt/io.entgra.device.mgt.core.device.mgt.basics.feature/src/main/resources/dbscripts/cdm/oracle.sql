...
CREATE TABLE DM_DEVICE_STATUS (
    ...
    PRIMARY KEY (ID),
    CONSTRAINT FK_DM_DEVICE_STATUS_DEVICE FOREIGN KEY (DEVICE_ID) REFERENCES DM_DEVICE (ID),
    CONSTRAINT FK_DM_DEVICE_STATUS_ENROLMENT FOREIGN KEY (ENROLMENT_ID) REFERENCES DM_ENROLMENT (ID) ON DELETE CASCADE
);
/
-- Generate ID using sequence and trigger
...
CREATE TABLE DM_NOTIFICATION (
    ...
    TENANT_ID NUMBER(10) NOT NULL,
    NOTIFICATION_CONTEXT VARCHAR2(1000) NULL,
...
CREATE OR REPLACE TRIGGER DM_NOTIFICATION_bir
    BEFORE INSERT ON DM_NOTIFICATION
    FOR EACH ROW
BEGIN
...
CREATE INDEX IDX_NOTF_CONFIG_ID ON DM_NOTIFICATION(NOTIFICATION_CONFIG_ID);
CREATE INDEX IDX_NOTF_TENANT_ID ON DM_NOTIFICATION(TENANT_ID);
CREATE INDEX IDX_NOTF_CREATED_TIMESTAMP ON DM_NOTIFICATION(CREATED_TIMESTAMP);
/

CREATE TABLE DM_NOTIFICATION_DEVICE (
    NOTIFICATION_ID NUMBER(10) NOT NULL,
    DEVICE_ID NUMBER(10) NOT NULL,
    CONSTRAINT PK_DM_NOTIFICATION_DEVICE PRIMARY KEY (NOTIFICATION_ID, DEVICE_ID),
    CONSTRAINT FK_DM_NOTIFICATION_DEVICE_NOTIFICATION FOREIGN KEY (NOTIFICATION_ID) REFERENCES DM_NOTIFICATION (NOTIFICATION_ID) ON DELETE CASCADE,
    CONSTRAINT FK_DM_NOTIFICATION_DEVICE_DEVICE FOREIGN KEY (DEVICE_ID) REFERENCES DM_DEVICE (ID) ON DELETE CASCADE
);

CREATE INDEX IDX_NOTF_DEV_DEVICE_ID ON DM_NOTIFICATION_DEVICE(DEVICE_ID);
/
-- NOTIFICATION TABLE END --

-- DM_NOTIFICATION_USER_ACTION TABLE --
CREATE TABLE DM_NOTIFICATION_USER_ACTION (
    ACTION_ID NUMBER(10) NOT NULL,
    NOTIFICATION_ID NUMBER(10) NOT NULL,
    USERNAME VARCHAR2(255) NOT NULL,
    IS_READ NUMBER(1) DEFAULT 0,
    ACTION_TIMESTAMP TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT PK_DM_NOTIFICATION_USER_ACTION PRIMARY KEY (ACTION_ID),
    CONSTRAINT FK_DM_NOTIFICATION_USER_ACTION_NOTIFICATION FOREIGN KEY (NOTIFICATION_ID) REFERENCES DM_NOTIFICATION (NOTIFICATION_ID)
);

CREATE SEQUENCE DM_NOTIFICATION_USER_ACTION_seq START WITH 1 INCREMENT BY 1 NOCACHE;

CREATE OR REPLACE TRIGGER DM_NOTIFICATION_USER_ACTION_bir
    BEFORE INSERT ON DM_NOTIFICATION_USER_ACTION
    FOR EACH ROW
BEGIN
    IF :NEW.ACTION_ID IS NULL THEN
        SELECT DM_NOTIFICATION_USER_ACTION_seq.NEXTVAL INTO :NEW.ACTION_ID FROM DUAL;
    END IF;
END;
/
...
-- DM_DEVICE_TYPE_META TABLE --
CREATE TABLE DM_DEVICE_TYPE_META (
    ID NUMBER(10) NOT NULL,
    DEVICE_TYPE_ID NUMBER(10) NOT NULL,
    META_KEY VARCHAR2(100) NOT NULL,
    META_VALUE VARCHAR2(20000),
    LAST_UPDATED_TIMESTAMP NUMBER NOT NULL,
    TENANT_ID NUMBER DEFAULT 0,
    PRIMARY KEY (ID),
    FOREIGN KEY (DEVICE_TYPE_ID) REFERENCES DM_DEVICE_TYPE(ID),
    CONSTRAINT UK_DM_DEVICE_TYPE_META UNIQUE (DEVICE_TYPE_ID, TENANT_ID, META_KEY)
);
-- Sequence for DM_DEVICE_TYPE_META AUTO_INCREMENT equivalent
CREATE SEQUENCE DM_DEVICE_TYPE_META_seq START WITH 1 INCREMENT BY 1;

-- Trigger for DM_DEVICE_TYPE_META AUTO_INCREMENT equivalent
CREATE OR REPLACE TRIGGER DM_DEVICE_TYPE_META_seq_tr
BEFORE INSERT
ON DM_DEVICE_TYPE_META
REFERENCING NEW AS NEW
FOR EACH ROW
WHEN (NEW.ID IS NULL)
BEGIN
SELECT DM_DEVICE_TYPE_META_seq.NEXTVAL
INTO :NEW.ORGANIZATION_ID
FROM ID;
END;
/

-- END OF DM_DEVICE_TYPE_META TABLE --


-- DM_OPERATION_DETAILS TABLE --
CREATE TABLE DM_OPERATION_DETAILS (
...
CREATE UNIQUE INDEX UQ_OPERATION_DEVICE ON DM_OPERATION_DETAILS (OPERATION_CODE, DEVICE_TYPE);
-- END OF DM_OPERATION_DETAILS TABLE --
